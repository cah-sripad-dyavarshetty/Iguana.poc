...
function main()
   ...
   if pcall(first_Function) then 
    ...
    if pcall(verifyAllDirectories) then   --if 114
...
function fun(filename)
   ...
   for filename in file_directory:lines() do
            ...
            if pcall(fun1,order_file,filename) then  --if 113
...
function fun1(order_file,filename)
            ...
            if(GetFileExtension(order_file) == '.xml') then -- Validation file extension
                ...
                if not open_order_file then log_file:write(TIME_STAMP..filename.." - "..UNABLE_OPEN_FILE..order_file,"\n") else
                    ...
                    if pcall(Parser) then  --if 112

                        local order_data = Parser()
                        local order_data_validation_status = validationForOrderData(order_data)
                     if pcall(fun2,order_data,filename,order_data_validation_status) then   --if 111
...
function fun2(order_data,filename,order_data_validation_status)
    ...
    if(order_data_validation_status==true) then -- order validation if condition
                            ...
                            if pcall(Verify_DBConn) then
                               if pcall(fun3,fileName_with_timestamp,filename) then  --if 100
                               else
                                   print("*********************hello***********************")
                               end  --end if 100
                            else
...
function fun3(fileName_with_timestamp,filename)
...
function validationForOrderData(order_data)
    ...
    then
      ...
      validateion_status=validation2(order_data,SIZE_OF_ORDERITEM)
...
function validation2(order_data,SIZE_OF_ORDERITEM,validateion_status)
